...
class IntegrateController extends Controller {
    ...
    {
        //添加分页的查询
        $res = Integrate::where('intVersionID','=',$version)->paginate(15);
...
//            $res->chrIntegrateDescribe = $data['chrIntegrateDescribe'];
    ...
    public  function getIntegrate(Request $request)
    {
        ...
        $integrate = Integrate::where('intVersionID','=',$version)->get();
        $arr=array();
        if($integrate){
            $data = $integrate;
            $arr['success'] = 1;
