IndexUtil = function(util) {
	...
	return me = {
		...
		loadApiTaskPies(){
			CommonUtil.requestService(chartUrl+"apiTaskPie", "", true, "get", function(response,
						status) {
				if (response.success) {
					var web=response.data.web;
					me.apiTaskPie(web);
		...
		apiTaskPie:function(web){
			web=CommonUtil.parseToJson(web);
			var lgData=[];
			var color=[];
			var webCount=0;
			for(var idx in web){
				$("#apiState .mission-"+web[idx].state+"").empty()
					.append("<span></span>"+web[idx].name+"<b>"+web[idx].value+"</b>");
				webCount+=parseInt(web[idx].value);
			}
			$("#apiTaskPieCount").text(webCount);
			$("#apiState li").each(function(){
				var value=$(this).find("b").text();
				if(value!="0"){
					lgData.push($(this).find("b").text());
					color.push($(this).find("span").css("background-color"));
			...
			// 基于准备好的dom，初始化echarts实例
			var myChart = echarts.init(document.getElementById('apiTaskPie'));
			// 指定图表的配置项和数据
			var option = {
				tooltip: {
					trigger: 'item',
		...
		loadAppTaskPies(){
			CommonUtil.requestService(chartUrl+"appTaskPie", "", true, "get", function(response,
						status) {
				if (response.success) {
					var web=response.data.web;
					me.appTaskPie(web);
		...
		appTaskPie:function(web){
			web=CommonUtil.parseToJson(web);
			var lgData=[];
			var color=[];
			var webCount=0;
			for(var idx in web){
				$("#appState .mission-"+web[idx].state+"").empty()
					.append("<span></span>"+web[idx].name+"<b>"+web[idx].value+"</b>");
				webCount+=parseInt(web[idx].value);
			}
			$("#appTaskPieCount").text(webCount);
			$("#appState li").each(function(){
				var value=$(this).find("b").text();
				if(value!="0"){
					lgData.push($(this).find("b").text());
					color.push($(this).find("span").css("background-color"));
			...
			// 基于准备好的dom，初始化echarts实例
			var myChart = echarts.init(document.getElementById('appTaskPie'));
			// 指定图表的配置项和数据
			var option = {
				tooltip: {
					trigger: 'item',
		...
		loadDispatchTaskPies(){
			CommonUtil.requestService(chartUrl+"dispatchTaskPie", "", true, "get", function(response,
						status) {
				if (response.success) {
					var web=response.data.web;
					me.dispatchTaskPie(web);
				}
			}, function(ex) {
			});
		},
		dispatchTaskPie:function(web){
			web=CommonUtil.parseToJson(web);
			var lgData=[];
			var color=[];
			var webCount=0;
			for(var idx in web){
				$("#dispatchState .mission-"+web[idx].state+"").empty()
					.append("<span></span>"+web[idx].name+"<b>"+web[idx].value+"</b>");
				webCount+=parseInt(web[idx].value);
			}
			$("#dispatchTaskPieCount").text(webCount);
			$("#dispatchState li").each(function(){
				var value=$(this).find("b").text();
				if(value!="0"){
					lgData.push($(this).find("b").text());
					color.push($(this).find("span").css("background-color"));
			...
			// 基于准备好的dom，初始化echarts实例
			var myChart = echarts.init(document.getElementById('dispatchTaskPie'));
			// 指定图表的配置项和数据
			var option = {
				tooltip: {
					trigger: 'item',
		...
		loadScriptCharts:function(){
			CommonUtil.requestService(chartUrl+"script", "", true, "get", function(response,
				...
				if (response.success) {
					...
					$("#apiScriptCount").text(web[1].value);
					$("#appScriptCount").text(web[2].value);
					$("#dispatchScriptCount").text(web[3].value);
				}
			}, function(ex) {
			});
		}
